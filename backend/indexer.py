...
MEILISEARCH_KEY = os.getenv("MEILISEARCH_KEY", "dev-master-key-change-later")
INDEX_NAME = "k8s-notes"

HEADINGS_PATTERN = re.compile(r"(?m)^(#{1,6})[ \t]+(.+?)[ \t]*$")
...
def configure_index(client: httpx.Client) -> None:
    ...
    response = client.patch(
        f"{MEILISEARCH_URL}/indexes/{INDEX_NAME}/settings/embedders",
        headers=meilisearch_headers(),
        json={
            "default": {
                "source": "userProvided",
...
def add_embeddings(
    ...
    for start in range(0, len(documents), batch_size):
        ...
        for document, embedding_item in zip(batch, response.data):
            document["_vectors"] = {
                "default": embedding_item.embedding
            }

        print(f"Embedded {min(start + batch_size, len(documents))}/{len(documents)} sections")
...
def main() -> None:
    ...
    if not documents:
        raise RuntimeError(f"No .md or .txt files found in {NOTES_DIR}")
